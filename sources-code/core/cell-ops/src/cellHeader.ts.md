---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/core/cell-ops/src/cellHeader.ts
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:17.824565+00:00
---

# core/cell-ops/src/cellHeader.ts

```ts
/**
 * Cell header packing + dimension-axis hash helpers.
 *
 * Historically named `typeHashRegistry.ts` and conflated two concerns:
 * (1) the composite typeHash for (WHAT,HOW,INST) triples — now owned by
 *     the canonical `buildTypeHash` primitive in `@semantos/protocol-types`
 *     and driven by `cartridges/<cartridge>/cartridge.json` `cellTypes[]`
 *     entries (T2.a, decision record §2 / §3), and
 * (2) the wire-format cell header builder/packer — which lives here.
 *
 * Renamed 2026-05-25 by T2.a tick 9 (Q9-A resolution) to reflect what
 * the file actually does.  The composite (WHAT,HOW,INST) typeHash registry
 * (`computeTypeHash`, `oddjobzTypeHash`, `ODDJOBZ_CELL_TYPE_IDENTITIES`,
 * `OddjobzCellTypeIdentity`) was deleted; callers use `buildTypeHash` from
 * `@semantos/protocol-types` and look up identities via the manifest-driven
 * cartridge registry.
 *
 * Dimension-axis hash helpers (`computeWhatHash`, `computeHowHash`,
 * `computeInstHash`, `computePhaseHash`) survive — they are generic
 * SHA256-of-prefixed-string utilities still used by capability tests
 * (`core/cell-engine/tests-bun/proof-of-capability.test.ts`).
 */

import { createHash } from "crypto";

// ── Dimension-axis hash helpers ──────────────────────
...
export interface CellHeader {
  ...
  phase: number;          //  1 byte  (offset 94)   — legacy commerce field; new callers use domainPayloadRoot
  dimension: number;      //  1 byte  (offset 95)   — legacy commerce field
  parentHash: Buffer;     // 32 bytes (offset 96)   — legacy commerce field
  prevStateHash: Buffer;  // 32 bytes (offset 128)  — legacy commerce field
  /** 32-byte SHA-256 of the encoded payload under its domain schema
   *  (Phase H §3.3 / RM-023). Stored at offset 224. Zero-filled for
   *  legacy cells written before the slot was added. */
  domainPayloadRoot: Buffer;
}
 ...
 *
 * RM-032 / RM-041 (partial): a new `domainPayload?: Buffer` arg lets
 * callers write a 32B `domainPayloadRoot` (Phase H §3.3) at header
 * offset 224. The legacy `phase`/`dimension`/`parentHash`/`prevStateHash`
 * args remain functional for back-compat — they continue to write to
 * the original commerce-shaped offsets (94/95/96/128). New callers
 * should prefer `domainPayload` + the `commerceSchemaV1` from
 * `@semantos/plexus-schema-registry/schemas/commerce`. The full strip
 * of the legacy args is scheduled for a follow-up RM-032b.
 */
export function buildCellHeader(opts: {
  typeHash: Buffer;
  linearity: Linearity;
  ownerId: Buffer;       // 16 bytes
  /** RM-041 / RM-032b: optional. Legacy commerce taxonomy that wrote
   *  to header offset 94 — new callers use `domainPayload` with a
   *  schema-encoded commerce payload instead. Old callers still
   *  pass it and the byte is written for back-compat. */
  phase?: PipelinePhase;
  /** RM-041 / RM-032b: optional, see `phase`. Writes at offset 95. */
  dimension?: Dimension;
  parentHash?: Buffer;   // 32 bytes, optional
  prevStateHash?: Buffer; // 32 bytes, optional
  /** 32-byte SHA-256 payload root for the cell's domain (Phase H §3.3 /
   *  RM-023). When set, written at header offset 224. Compute via
   *  `computeDomainPayloadRoot(schema, values)` from
   *  `@semantos/plexus-schema-registry`. */
  domainPayload?: Buffer;
  payloadSize: number;
  version?: number;
}): Buffer {
  ...
  // Phase (offset 94, 1 byte) — legacy commerce taxonomy; only written
  // when supplied. New callers compute commerce semantics into the
  // payload via commerceSchemaV1 and pass `domainPayload`.
  if (opts.phase !== undefined) {
    header.writeUInt8(PHASE_BYTES[opts.phase], 94);
  }

  // Dimension (offset 95, 1 byte) — legacy, see `phase`.
  ...
  // DomainPayloadRoot (offset 224, 32 bytes) — Phase H §3.3 / RM-023.
  if (opts.domainPayload) {
    opts.domainPayload.copy(header, 224, 0, 32);
  }
...
export function unpackCell(cell: Buffer): { header: CellHeader; payload: Buffer } {
  ...
  const domainPayloadRoot = cell.subarray(224, 256);
  ...
  return {
    header: {
      ...
      domainPayloadRoot: Buffer.from(domainPayloadRoot),
    },
