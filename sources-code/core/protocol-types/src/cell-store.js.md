---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/core/protocol-types/src/cell-store.js
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:17.851217+00:00
---

# core/protocol-types/src/cell-store.js
...
import { CELL_SIZE, HEADER_SIZE, PAYLOAD_SIZE, CONTINUATION_HEADER_SIZE, CONTINUATION_PAYLOAD_SIZE, MAGIC_1, MAGIC_2, MAGIC_3, MAGIC_4, Linearity, CommercePhase, CellType, } from './constants';
// ── SHA-256 Helper ────────────────────────────────────────────────
 ...
 */
function buildContinuationHeader(cellType, cellIndex, totalCells, payloadSize) {
...
function parseContinuationHeader(cell) {
...
export class CellStore {
    ...
    async put(key, data, options) {
        ...
        if (options?.prevStateHash) {
            prevStateHash = options.prevStateHash;
        }
        else if (prevMeta) {
            prevStateHash = hexToBytes(prevMeta.cellHash);
        }
        const now = Date.now();
        ...
        if (isChunked) {
            ...
            if (manifestBytes.length > PAYLOAD_SIZE) {
                throw new Error(`Manifest too large: ${manifestBytes.length} bytes exceeds ${PAYLOAD_SIZE}. Too many chunks.`);
            }
        ...
        if (isChunked && manifest) {
            for (let i = 0; i < manifest.chunkCount; i++) {
                ...
                const contHeader = buildContinuationHeader(CellType.DATA, i + 1, // 1-based index
        ...
        await this.adapter.write(`${key}.meta`, new TextEncoder().encode(JSON.stringify(meta)));
    ...
    async get(key) {
        ...
        if (!cellBytes || cellBytes.length < CELL_SIZE)
            return null;
        ...
        if (header.cellCount > 1) {
            ...
            for (let i = 0; i < manifest.chunkCount; i++) {
                ...
                const chunkData = chunkCell.subarray(CONTINUATION_HEADER_SIZE, CONTINUATION_HEADER_SIZE + contHeader.payloadSize);
        ...
        if (meta) {
            ...
            while (currentVersion >= 1) {
                ...
                refs.push({
                    ...
                    linearity: versionedMeta.linearity,
                });
                currentVersion--;
            }
        }
    ...
    async findByContent(contentHash) {
        ...
        if (!indexBytes)
            return [];
        try {
            const entries = JSON.parse(new TextDecoder().decode(indexBytes));
    ...
    async updateContentIndex(contentHash, entry) {
        ...
        await this.adapter.write(indexKey, new TextEncoder().encode(JSON.stringify(entries)));
    }
}
