---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/core/protocol-types/src/cell-token.ts
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:17.844994+00:00
---

# core/protocol-types/src/cell-token.ts
    ...
    tx.addInput({
      ...
      unlockingScriptTemplate: {
        sign: async (tx: Transaction, inputIndex: number): Promise<UnlockingScript> => {
          // TODO: API-drift with @bsv/sdk 2.x — `getSignaturePreimage` was renamed
          // to `preimage` and the signature changed. This helper is unused at
          // runtime (kept for reference); @ts-expect-error markers keep strict
          // importers (chain-broadcast) compiling. Fix the API call when this
          // helper gets wired into something.
          // @ts-expect-error pre-existing @bsv/sdk API drift
          ...
          // @ts-expect-error sigDer is `number[] | string`; spread works at runtime, type-only drift
          const sigWithHashType: number[] = [...sigDer, 0x41];
