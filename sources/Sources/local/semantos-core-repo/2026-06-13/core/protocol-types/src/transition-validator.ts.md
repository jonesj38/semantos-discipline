---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/core/protocol-types/src/transition-validator.ts
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:17.839475+00:00
---

# core/protocol-types/src/transition-validator.ts
...
import { TypeClassification } from '@semantos/cell-ops';
...
export class TransitionValidator {
  ...
  validate(input: TransitionInput): TransitionValidationResult {
    ...
    //
    // v2.prevStateHash must equal sha256(v1CellBytes). This binds every
    ...
    // (RM-032b renamed the slot from `commercePrevState` to `prevStateHash`;
    // chain semantics unchanged.)
    if (isZeroBytes(v2Header.prevStateHash)) {
      return this.fail(
        `v2.prevStateHash is zero — successor cells must bind to their ` +
    ...
    if (!bytesEqual(v2Header.prevStateHash, expectedPrevCellHash)) {
      return this.fail(
        ...
        `v2.prevStateHash=${bytesToHex(v2Header.prevStateHash).slice(0, 16)}... ` +
