---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/cartridges/oddjobz/brain/zig/web_chat_http.zig
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:18.479218+00:00
---

# cartridges/oddjobz/brain/zig/web_chat_http.zig

```zig
// C4 CW-1 — oddjobz web-chat intake exec helper (cartridge-owned).
//
// Carved from the brain's generic src/intake_http.zig: this is the
// cartridge's own copy of the parse + bun-exec logic for the public web
// chat widget, so the `/api/v1/chat` route is a cartridge route_registry
// route (registration.zig::chatRouteHandler) rather than the generic brain
// `intake` site-route kind. The brain's intake_http.zig + RouteType.intake
// are retired in CW-4 once this is the sole consumer.
//
...
//
// The caller may pass an optional `entity_cell_hash` (a 64-hex job cell ID
// sourced from the `?j=<cellId>` query param). When present the
// intake-handler sets entityRef on the written ConversationTurn so the
// widget chat is anchored to the correct job.
//
// No persistent process — one subprocess per request. Acceptable for the
// personal-scale sovereign node; a long-lived sidecar with a Unix socket
// can replace this later without changing the route shape.
...
pub fn parseIntakeRequest(allocator: std.mem.Allocator, body: []const u8) ParseError!IntakeRequest {
    ...
    // optional entity_cell_hash forwarded from the ?j= query param.
...
pub fn callScript(
    ...
    // diagnostic. Inherit stderr so the handler's best-effort logs —
    // submitLeadCell diagnostics, recordIntakeTurn/persistLead errors — land in
    // the brain's stderr/journal. .Inherit (vs .Pipe) needs no draining, so it
    // can't deadlock against the bounded 128 KB stdout read loop below.
