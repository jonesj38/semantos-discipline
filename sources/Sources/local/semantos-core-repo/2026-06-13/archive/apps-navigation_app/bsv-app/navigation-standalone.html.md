---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/archive/apps-navigation_app/bsv-app/navigation-standalone.html
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:18.724761+00:00
---

# archive/apps-navigation_app/bsv-app/navigation-standalone.html
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── Dashboard ── */
    .dash-pad { padding: 20px 16px; gap: 16px; }
    .greeting { font-size: 24px; font-weight: 700; }
    .greeting-sub { font-size: 13px; color: var(--text-50); margin-top: 4px; }

    .streak-badge {
      display: inline-flex; align-items: center; gap: 8px;
      background: rgba(249,115,22,0.15); border-radius: var(--radius-pill);
      padding: 8px 16px; font-size: 14px; font-weight: 600; color: var(--orange);
    }
    ...
    .dim-emoji { font-size: 18px; width: 28px; text-align: center; }
    .dim-label { font-size: 13px; color: var(--text-70); width: 80px; }
    .dim-bar-wrap {
      flex: 1; height: 6px; background: var(--text-10);
    ...
    .dim-score { font-size: 13px; font-weight: 600; width: 28px; text-align: right; }
    ...
    .quick-btn:active { background: var(--surface-active); }
    ...
    /* ── Process Map ── */
    .process-pad { padding: 16px; }
    .process-header { font-size: 20px; font-weight: 700; margin-bottom: 4px; }
    .process-sub { font-size: 13px; color: var(--text-50); margin-bottom: 16px; }
    ...
    .cycle-card:active { background: var(--surface-active); }
    .cycle-title { font-size: 16px; font-weight: 600; margin-bottom: 2px; }
    .cycle-inquiry { font-size: 13px; font-style: italic; color: var(--text-50); margin-bottom: 8px; }
    .cycle-desc { font-size: 13px; color: var(--text-70); margin-bottom: 10px; line-height: 1.4; }
    .cycle-flow { display: flex; flex-wrap: wrap; gap: 6px; align-items: center; }
    .step-chip {
      font-size: 11px; padding: 4px 10px; border-radius: var(--radius-pill);
      font-weight: 500;
    }
    .step-chip.neutral { background: var(--surface); color: var(--text-50); }
    .step-chip.release { background: rgba(239,68,68,0.15); color: var(--red); }
    .step-chip.receive { background: rgba(74,222,128,0.15); color: var(--green); }
    .flow-arrow { color: var(--text-30); font-size: 12px; }

    /* ── Insights ── */
    .insights-pad { padding: 16px; }
    .insight-tabs {
    ...
    .insight-tab {
      ...
      color: var(--text-30); background: none; border: none;
      cursor: pointer; border-bottom: 2px solid transparent;
      transition: color 0.2s, border-color 0.2s;
    }
    .insight-tab.active { color: var(--text); border-bottom-color: var(--blue); }

    .insight-card {
      background: var(--surface); border-radius: var(--radius);
      padding: 14px 16px; margin-bottom: 10px;
    }
    .insight-card.gradient-card {
      background: linear-gradient(135deg, #1e1b4b, #172554);
    }
    .insight-content { font-size: 14px; line-height: 1.5; color: var(--text); }
    .insight-meta { display: flex; gap: 8px; margin-top: 8px; align-items: center; }
    .source-chip {
      font-size: 11px; padding: 2px 8px; border-radius: var(--radius-pill);
      font-weight: 500;
    }
    .source-chip.writing { background: rgba(59,130,246,0.15); color: var(--blue); }
    .source-chip.connection { background: rgba(74,222,128,0.15); color: var(--green); }
    .source-chip.llm { background: rgba(139,92,246,0.15); color: var(--purple); }
    ...
    .pair-dims { font-size: 13px; color: var(--text-70); flex: 1; }
    .pair-icon { font-size: 16px; }
    .pair-strength { font-size: 14px; font-weight: 600; }
    .pair-strength.reinforcing { color: var(--green); }
    .pair-strength.tension { color: var(--amber); }

    .pattern-bar-wrap {
      height: 4px; background: var(--text-10); border-radius: 2px;
      margin-top: 8px; overflow: hidden;
    }
    .pattern-bar { height: 100%; border-radius: 2px; background: var(--amber); }
    .pattern-count { font-size: 11px; color: var(--text-30); margin-top: 4px; }

    /* ── Evening Review / Morning Intention overlays ── */
    .overlay {
    ...
    .overlay-header {
      display: flex; align-items: center; justify-content: space-between;
      padding: 12px 16px; border-bottom: 1px solid var(--text-10);
    }
    ...
    /* Progress steps */
    .progress-bar {
      display: flex; gap: 3px; padding: 0 16px; margin-top: 8px;
    }
    .progress-seg {
      flex: 1; height: 3px; border-radius: 2px;
      background: var(--text-10); transition: background 0.3s;
    }
    .progress-seg.done { background: var(--blue); }

    /* Form fields */
    .form-label { font-size: 16px; font-weight: 600; margin-bottom: 4px; }
    .form-hint { font-size: 13px; color: var(--text-50); margin-bottom: 12px; }
    .form-input {
    ...
    .slider-emoji { font-size: 20px; width: 28px; text-align: center; }
    ...
    .slider-wrap input[type=range]::-webkit-slider-runnable-track {
      height: 4px; border-radius: 2px; background: var(--text-10);
    }
    ...
    .slider-val { font-size: 16px; font-weight: 600; width: 28px; text-align: center; }

    /* Dimension picker grid */
    .dim-grid {
    ...
    .dim-pick {
      ...
      border: 2px solid var(--text-10); background: var(--surface);
      cursor: pointer; transition: border-color 0.2s, background 0.2s;
    }
    .dim-pick:active, .dim-pick.selected {
      border-color: var(--blue); background: rgba(59,130,246,0.08);
    }
    .dim-pick .dp-emoji { font-size: 28px; }
    .dim-pick .dp-label { font-size: 13px; color: var(--text-70); }
    ...
    .btn-subtle { background: var(--surface); color: var(--text-70); }

    /* ── Release writing view ── */
    .release-area {
      flex: 1; display: flex; flex-direction: column; padding: 16px;
    }
    .release-timer {
      text-align: center; font-family: monospace; font-size: 18px;
      color: var(--blue); padding: 8px; margin-bottom: 12px;
    }
    .release-textarea {
      flex: 1; background: none; color: var(--text); border: none;
      font-size: 17px; line-height: 1.7; font-family: inherit;
      resize: none; outline: none;
    }
    .release-textarea::placeholder { color: var(--text-30); }
    .release-footer {
      display: flex; align-items: center; justify-content: space-between;
      padding: 12px 0;
    }
    .release-wc { font-size: 13px; color: var(--text-30); }
    .release-btn {
      padding: 10px 24px; border-radius: var(--radius-pill);
      border: none; background: rgba(239,68,68,0.15); color: var(--red);
      font-size: 14px; font-weight: 600; cursor: pointer;
    }
    .release-btn:active { background: rgba(239,68,68,0.25); }

    /* Prompt chips */
    .prompt-chips {
      display: flex; gap: 8px; overflow-x: auto; padding-bottom: 12px;
    }
    .prompt-chips::-webkit-scrollbar { height: 0; }
    .prompt-chip {
      white-space: nowrap; padding: 6px 14px; border-radius: var(--radius-pill);
      border: 1px solid var(--text-10); background: none;
      color: var(--text-50); font-size: 13px; cursor: pointer;
      transition: background 0.2s;
    }
    .prompt-chip:active { background: var(--surface); }
...
<div class="app">
  ...
  <!-- ═══ Dashboard View ═══ -->
  <div class="view active" id="home-view" style="gap:0;">
    <div class="dash-pad" id="dashboard-content">
      <!-- Filled by JS -->
    </div>
  </div>
  ...
  <div class="view chat-view" id="chat-view">
    ...
    <div class="chat-input-area">
      ...
      <textarea id="input" rows="1" placeholder="What's on your mind..."
  ...
  <!-- ═══ Process View ═══ -->
  <div class="view" id="process-view">
    <div class="process-pad" id="process-content"></div>
  </div>

  <!-- ═══ Insights View ═══ -->
  <div class="view" id="insights-view">
    <div class="insights-pad" id="insights-content"></div>
  </div>

  <!-- ═══ Bottom Nav ═══ -->
  <nav class="nav">
    <button class="nav-item active" data-view="home" onclick="app.switchView('home')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      Home
    </button>
    <button class="nav-item" data-view="chat" onclick="app.switchView('chat')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
      Talk
    </button>
    <button class="nav-item" data-view="process" onclick="app.switchView('process')">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
      Process
    </button>
    <button class="nav-item" data-view="insights" onclick="app.switchView('insights')">
  ...
  <div class="overlay" id="review-overlay">
    <div class="overlay-header">
      <span class="overlay-title">Evening Check-in</span>
      <button class="overlay-close" onclick="app.closeOverlay('review')">&times;</button>
    </div>
    <div class="progress-bar" id="review-progress"></div>
    <div class="overlay-body" id="review-body"></div>
  </div>

  <!-- ═══ Morning Intention Overlay ═══ -->
  <div class="overlay" id="intention-overlay">
    <div class="overlay-header">
      <span class="overlay-title">Morning Intention</span>
      <button class="overlay-close" onclick="app.closeOverlay('intention')">&times;</button>
    </div>
    <div class="overlay-body" id="intention-body"></div>
  </div>

  <!-- ═══ Release Overlay ═══ -->
  <div class="overlay" id="release-overlay">
    <div class="overlay-header">
      <span class="overlay-title">Release</span>
      <button class="overlay-close" onclick="app.closeOverlay('release')">&times;</button>
    </div>
    <div class="overlay-body" style="padding:0;">
      <div class="release-area">
        <div class="release-timer" id="release-timer">00:00</div>
        <div class="prompt-chips">
          <button class="prompt-chip" onclick="app.insertPrompt('What am I holding onto?')">What am I holding onto?</button>
          <button class="prompt-chip" onclick="app.insertPrompt('What would I release if I could?')">What would I release?</button>
          <button class="prompt-chip" onclick="app.insertPrompt('Where is there resistance?')">Where is there resistance?</button>
          <button class="prompt-chip" onclick="app.insertPrompt('What does my highest self know?')">What does my highest self know?</button>
        </div>
        <textarea class="release-textarea" id="release-text"
          placeholder="Write freely. Let it flow. This will be released — consumed by the kernel and gone forever."
          oninput="app.onReleaseType()"></textarea>
        <div class="release-footer">
          <span class="release-wc" id="release-wc">0 words</span>
          <button class="release-btn" id="release-btn" onclick="app.commitRelease()">Release ↗</button>
        </div>
      </div>
    </div>
  </div>
...
const app = {
  ...
  renderDashboard() {
    ...
    // Recent activity
    const recent = Object.values(this.objects).slice(-3).reverse();
    if (recent.length > 0) {
      html += '<div class="card" style="margin-top:4px">';
      html += '<div class="card-title">Recent</div>';
      for (const obj of recent) {
        const icon = this.objectIcon(obj.type);
        const label = this.objectLabel(obj);
        const time = this.timeAgo(obj.createdAt);
...
- Be honest. If they're avoiding something, gently note it.`;
  ...
  localProcess(message) {
    ...
    if (lower.includes('release') || lower.includes('let go') || lower.includes('i feel') || lower.includes('frustrated') || lower.includes('anxious') || lower.includes('stressed')) {
      const release = {
        id: `rel-${Date.now()}`, type: 'Release', linearity: 'LINEAR',
        payload: { rawText: message, themes: [], emotionalValence: 0 },
        createdAt: new Date().toISOString(),
      };
      this.objects[release.id] = release;
      return {
        reply: 'I hear you. That\'s been released — it\'s out of you now. What themes were in there for you?',
        actions: [{ type: 'create', objectType: 'Release', fields: release.payload }],
      };
    }
    ...
    if (lower.includes('insight') || lower.includes('realised') || lower.includes('realized') || lower.includes('i see that') || lower.includes('it hit me')) {
      const insight = {
        id: `ins-${Date.now()}`, type: 'Insight', linearity: 'RELEVANT',
        payload: { content: message, source: 'writing', tags: [] },
        createdAt: new Date().toISOString(),
      };
      this.objects[insight.id] = insight;
      return {
        reply: 'That\'s a real insight. I\'ll keep that one — it might connect with patterns down the track.',
        actions: [{ type: 'create', objectType: 'Insight', fields: insight.payload }],
      };
    }
  ...
  commitRelease() {
    ...
    this.objects[id] = {
      ...
      payload: {
        ...
        themes: [],
        emotionalValence: 0,
        durationSeconds: this.releaseSeconds,
      },
      createdAt: new Date().toISOString(),
      consumed: true,
    };
  ...
  saveReview() {
    ...
    this.objects[id] = {
      ...
      payload: {
        wins: this.reviewData.wins.filter(w => w.trim()),
        improvements: this.reviewData.improvements.filter(i => i.trim()),
        tomorrowIntention: this.reviewData.intention,
        energyLevel: this.reviewData.energy,
        moodLevel: this.reviewData.mood,
        dimensionScores: { ...this.reviewData.dimensions },
      },
      createdAt: new Date().toISOString(),
      consumed: true,
    };
  ...
  saveIntention() {
    ...
    this.objects[id] = {
      ...
      payload: {
        focusDimension: this.intentionData.dimension,
  ...
  renderInsights() {
    ...
    const insights = Object.values(this.objects).filter(o => o.type === 'Insight');
    const patterns = Object.values(this.objects).filter(o => o.type === 'Pattern');
    ...
    if (this.insightTab === 'insights') {
      if (insights.length === 0) {
        ...
        for (const ins of insights.reverse()) {
          const source = ins.payload?.source || 'writing';
          html += `
            <div class="insight-card">
              <div class="insight-content">${this.esc(ins.payload?.content || '')}</div>
              <div class="insight-meta">
                <span class="source-chip ${source}">${source}</span>
                <span class="time-ago">${this.timeAgo(ins.createdAt)}</span>
              </div>
            </div>`;
        }
      }
      ...
      if (patterns.length === 0) {
        ...
        for (const pat of patterns) {
          const strength = (pat.payload?.strength || 0) * 100;
          html += `
            <div class="insight-card">
              <div class="insight-content">${this.esc(pat.payload?.description || '')}</div>
              <div class="pattern-bar-wrap"><div class="pattern-bar" style="width:${strength}%"></div></div>
              <div class="pattern-count">${pat.payload?.occurrences || 0}× observed</div>
            </div>`;
        }
      }
    } else {
