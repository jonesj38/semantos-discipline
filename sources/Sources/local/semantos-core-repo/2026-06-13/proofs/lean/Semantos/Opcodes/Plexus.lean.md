---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/proofs/lean/Semantos/Opcodes/Plexus.lean
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:17.359616+00:00
---

# proofs/lean/Semantos/Opcodes/Plexus.lean
...
def validDemotion (src tgt : Linearity) : Bool :=
  match src, tgt with
  | .linear, .affine   => true
...
/-- Dispatch a Plexus opcode (0xC0-0xCF).
    Matches plexus.zig executePlexus (lines 42-62).

    The full dispatch table covers all 16 opcodes 0xC0-0xCF:
      0xC0-0xC8: structural / type-check / dereference (Plexus.lean above)
      0xC9-0xCC: header/payload reads, cell creation, demotion (above)
      0xCD:      OP_SIGN (Sign.lean) — wallet tier-key signing
      0xCE-0xCF: budget debit/refill (Budget.lean)

    Three host-style oracles are taken as parameters, paralleling how
    `opDerefPointer` already takes `hostFetch`:
      - `hostFetch`: pointer-cell lookup (0xC8)
      - `budgetCheck`: amount validity for budget cells (0xCE/0xCF)
      - `checksig`: parent ECDSA verification for refills (0xCF)

    Any opcode outside 0xC0-0xCF returns `reservedOpcode`. -/
def executePlexus (op : Opcode) (pda : PDA)
    (hostFetch : Cell → Option Cell)
    (budgetCheck : Cell → Cell → Bool)
    (checksig : Cell → Cell → Cell → Bool) : Except OpcodeError PDA :=
  ...
  else if op == OP_SIGN then opSign pda
  else if op == OP_DECREMENT_BUDGET then opDecrementBudget pda budgetCheck
  else if op == OP_REFILL_BUDGET then opRefillBudget pda budgetCheck checksig
  else .error .reservedOpcode  -- unmapped (0xD1+ outside Plexus range)
