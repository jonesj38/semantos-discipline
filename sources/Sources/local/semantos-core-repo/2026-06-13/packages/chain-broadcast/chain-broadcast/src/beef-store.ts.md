---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/packages/chain-broadcast/chain-broadcast/src/beef-store.ts
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:17.520777+00:00
---

# packages/chain-broadcast/chain-broadcast/src/beef-store.ts

```ts
/**
 * BeefStore — Durable BEEF envelope persistence for the chain-broadcast pipeline.
 *
 ...
 *   - Per-tx merkle proofs (MerklePath / BUMP) when txs confirm
 *
 ...
 *
 * Advantages over a JSON chaintip:
 *   1. BEEF stores each parent tx once (automatic dedupe)
 *   2. Structural validity via Beef.isValid()
 *   3. Merkle proofs merge into the same Beef when txs confirm
 *   4. SPV verification via Beef.verify(chainTracker)
 *   5. AtomicBEEF extraction for per-handoff envelopes
 *
 * Ported as-is from todriguez/hackathon-submission:src/agent/beef-store.ts
 * (commit 496ee8f) — import paths updated for the newer @bsv/sdk layout
 * (ChainTracker is re-exported from the top-level barrel).
 *
 * Cross-references:
 ...
 *   core/cell-engine/src/beef.zig — native BEEF verifier (same wire format)
 */

import { Beef, Transaction, MerklePath, type ChainTracker } from "@bsv/sdk";
import {
  writeFileSync,
  readFileSync,
  existsSync,
...
export class BeefStore {
  ...
  constructor(config: BeefStoreConfig) {
    ...
    this.log =
      config.log ?? ((tag, msg) => console.log(`[${tag}] ${msg}`));
    ...
    this.flushTimer = setInterval(() => {
      if (this.dirty) {
        try {
          ...
          this.log("BEEF-STORE", `Persist failed: ${msg}`);
        }
      }
    }, interval);
    ...
    this.log(
      "BEEF-STORE",
      `initialized → ${config.filePath} (flush every ${interval}ms)`,
    );
  }
  ...
  /**
   * Verify the BEEF structure is valid. With `allowTxidOnly=true`, unconfirmed
   * ancestor chains are accepted.
   */
  ...
  /**
   * SPV-verify the BEEF against a configured ChainTracker. Only succeeds if
   * all root txs have valid merkle proofs matching confirmed block headers.
   */
  async verifySPV(): Promise<boolean> {
    if (!this.chainTracker) {
      this.log(
        "BEEF-STORE",
        "No ChainTracker configured — SPV verification skipped",
      );
      return false;
    }
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      this.log("BEEF-STORE", `SPV verification failed: ${msg}`);
      return false;
    }
  }
   ...
   * Call when a tx confirms and you have the BUMP.
   */
  ...
  /**
   * Get AtomicBEEF for a specific txid — a minimal BEEF containing only the
   * ancestry chain needed to verify that one tx.
   */
  ...
  restore(): boolean {
    ...
    try {
      ...
      this.log(
        "BEEF-STORE",
        `Restored ${restored.txs.length} txs from BEEF (${raw.length} bytes)`,
      );
      return true;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      this.log("BEEF-STORE", `Restore failed: ${msg}`);
      return false;
    }
  }
  ...
  getStats(): { txCount: number; fileSize: number; valid: boolean } {
    ...
    try {
      ...
      /* best-effort stat; surface zero on failure */
    }
