---
source_path: /home/jake/.edwinpai/disciplines/semantos/state/semantos-core-repo/runtime/shell/src/lisp/types.ts
source_type: folder
memory_type: semantic_memory
ingested_at: 2026-06-13T06:27:18.370905+00:00
---

# runtime/shell/src/lisp/types.ts

```ts
/**
 * Policy type system for the Lisp axiom compiler.
 *
 * Surface-grammar AST primitives (ConstraintExpr, IdentityRef,
 * ComparisonOp, LinearityMode + sub-types) live in
 * @semantos/semantos-ir/src/expr — they're consumed by the OIR/SIR
 * lowering pipeline and belong in core/. Re-exported here so existing
 * shell-side imports of './types' or '@semantos/shell/lisp/types'
 * continue resolving.
 *
 * Lisp-only types (PolicyForm, ScriptOutput) and the helper functions
 * (interpretConstraint, interpretPolicy, validateConstraintFields)
 * stay in this file because they're shell-internal.
 */

import type { FieldDefinition } from '@semantos/protocol-types';
import type { SExpression } from './parser';
import type {
  ConstraintExpr,
  IdentityRef,
  LinearityMode,
} from '@semantos/semantos-ir/expr';

// Re-export the hoisted types so existing consumers don't break.
export type {
  IdentityRef,
  ComparisonOp,
  ComparisonExpr,
